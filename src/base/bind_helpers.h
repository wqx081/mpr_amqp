...
//
//    scoped_ref_ptr<RefCountedBytes> bytes = ...;
//    Closure callback = Bind(&foo, base::RetainedRef(bytes));
//    callback.Run();
//
// Without RetainedRef, the scoped_ref_ptr would try to implicitly convert to
...
class RetainedRefWrapper {
 public:
  ...
  explicit RetainedRefWrapper(scoped_ref_ptr<T> o) : ptr_(std::move(o)) {}
  T* get() const { return ptr_.get(); }
 private:
  scoped_ref_ptr<T> ptr_;
};
...
template <typename T>
static inline internal::RetainedRefWrapper<T> RetainedRef(scoped_ref_ptr<T> o) {
